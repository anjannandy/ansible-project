...
# Wrapper script for running individual Kubernetes setup steps
set -e
...
export LOG_FILE="${LOG_FILE:-/tmp/k8s-setup.log}"
export STATE_DIR="${STATE_DIR:-/tmp/k8s-state}"
export LOG_DIR="${LOG_DIR:-/tmp}"
export MAX_RETRIES="${MAX_RETRIES:-3}"
export K8S_VERSION="${K8S_VERSION:-1.30.0-1.1}"
export POD_CIDR="${POD_CIDR:-10.244.0.0/16}"
export MASTER_IP="${MASTER_IP:-192.168.1.190}"
export KUBEADM_TIMEOUT="${KUBEADM_TIMEOUT:-600}"
export APT_TIMEOUT="${APT_TIMEOUT:-300}"
export NODE_ROLE="${NODE_ROLE:-worker}"
export MASTER_HOST="${MASTER_HOST:-192.168.1.190}"
...
log_info "  POD_CIDR: $POD_CIDR"
log_info "  MASTER_IP: $MASTER_IP"
...
# Kubernetes installation function
install_kubernetes() {
...
EOF
    ...
    if sudo timeout "$KUBEADM_TIMEOUT" kubeadm init --config=/tmp/kubeadm-config.yaml --ignore-preflight-errors=all --v=2; then
        ...
        sudo journalctl -xeu kubelet --no-pager -l | tail -50 || true
        return 1
    fi
...
case "$STEP" in
    "system_prep")
        log_info "=== STEP 1: System Preparation Starting ==="
        if system_prep; then
            log_success "STEP 1: System Preparation completed successfully"
            mark_complete "01_system_prep"
        else
            log_error "STEP 1: System Preparation failed"
            exit 1
        fi
        ;;
    "install_docker")
        log_info "=== STEP 2: Docker Installation Starting ==="
        if install_docker; then
            log_success "STEP 2: Docker Installation completed successfully"
            mark_complete "02_docker_install"
        else
            log_error "STEP 2: Docker Installation failed"
            exit 1
        fi
        ;;
    "install_kubernetes")
        log_info "=== STEP 3: Kubernetes Installation Starting ==="
        if install_kubernetes; then
            log_success "STEP 3: Kubernetes Installation completed successfully"
            mark_complete "03_k8s_install"
        else
            log_error "STEP 3: Kubernetes Installation failed"
            exit 1
        fi
        ;;
    "configure_system")
        log_info "=== STEP 4: System Configuration Starting ==="
        if configure_system; then
            log_success "STEP 4: System Configuration completed successfully"
            mark_complete "04_system_config"
        else
            log_error "STEP 4: System Configuration failed"
            exit 1
        fi
        ;;
    "initialize_master")
        log_info "=== STEP 5: Master Initialization Starting ==="
        if initialize_master; then
            log_success "STEP 5: Master Initialization completed successfully"
            mark_complete "05_master_init"
        else
            log_error "STEP 5: Master Initialization failed"
            exit 1
        fi
        ;;
    "join_worker")
        log_info "=== STEP 5: Worker Join Starting ==="
        if join_worker; then
            log_success "STEP 5: Worker Join completed successfully"
            mark_complete "05_worker_join"
        else
            log_error "STEP 5: Worker Join failed"
            exit 1
        fi
        ;;
    "install_cni")
        log_info "=== STEP 6: CNI Installation Starting ==="
        export KUBECONFIG="/home/ubuntu/.kube/config"
        if install_cni; then
            log_success "STEP 6: CNI Installation completed successfully"
            mark_complete "06_cni_install"
        else
            log_error "STEP 6: CNI Installation failed"
            exit 1
        fi
        ;;
    "verify_installation")
        log_info "=== FINAL: Verification Starting ==="
        if verify_installation; then
            log_success "FINAL: Verification completed successfully"
            echo "VERIFICATION_SUCCESS" > "${STATE_DIR}/verification_status"
        else
            log_warning "FINAL: Verification completed with warnings"
            echo "VERIFICATION_WARNING" > "${STATE_DIR}/verification_status"
        fi
        ;;
    *)
        echo "Usage: $0 {system_prep|install_docker|install_kubernetes|configure_system|initialize_master|join_worker|install_cni|verify_installation}"
